...
function RowMenu({ id, ativo, onEdit, onView, onToggle }: {
  id: string; ativo: boolean; onEdit: () => void; onView: () => void; onToggle: () => void;
}) {
...
export function FinalidadesPage() {
  const navigate = useNavigate();
  const [items, setItems]       = useState<Finalidade[]>([]);
  ...
  const load = async () => {
    ...
    const res = await fetch('/api/finalidades', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const execToggle = async () => {
    ...
    if (item.ativo) {
      ...
      await fetch(`/api/finalidades/${item.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ id: item.id, nome: item.nome, descricao: item.descricao, ativo: true }),
      });
    }
    load();
  };
