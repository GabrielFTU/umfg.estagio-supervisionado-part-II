...
interface CondicaoPagamento {
  ...
  diasParaPrimeiroVencimento: number; ativo: boolean;
}

const PAGE_SIZE_OPTIONS = [10, 20, 50];

function RowMenu({ item, onEdit, onView, onToggle }: {
  item: CondicaoPagamento; onEdit: () => void; onView: () => void; onToggle: () => void;
}) {
  return (
    <RowActionsMenu buttonClassName="text-gray-400 hover:text-gray-600" menuClassName="w-40">
      {close => (
        <>
...
export function CondicoesPagamentoPage() {
  ...
  const load = async () => {
    setLoading(true);
    const res = await fetchWithAuth('/api/condicoes-pagamento');
  ...
  const execConfirm = async () => {
    ...
    if (item.ativo) {
      await fetchWithAuth(`/api/condicoes-pagamento/${item.id}`, { method: 'DELETE' });
    } else {
      const res = await fetchWithAuth(`/api/condicoes-pagamento/${item.id}`);
      const data = await res.json();
      await fetchWithAuth(`/api/condicoes-pagamento/${item.id}`, {
        method: 'PUT',
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="shrink-0 px-6 py-4 border-b border-gray-100 flex items-center gap-4">
        ...
        <button onClick={() => navigate('/cadastros/condicoes-pagamento/novo')}
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                <tr key={item.id}
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">{item.numeroParcelas}x</td>
                  <td className="px-4 py-3 text-sm text-gray-500">{item.diasParaPrimeiroVencimento} dias</td>
