...
export function PessoasPage() {
  ...
  const filtered = pessoas.filter(p => {
    ...
    if (filtros.status === 'ativo'   && !p.ativo)                    return false;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="shrink-0 px-4 sm:px-6 pt-4 pb-3 bg-white border-b border-gray-200/70">
        <div className="flex items-center gap-1.5 text-xs text-gray-400">
          ...
          <span className="text-gray-600 font-medium">Pessoas</span>
        </div>
      </div>
          ...
          <div ref={filterRef} className="relative shrink-0">
            ...
            {filterOpen && (
              ...
              >
                ...
                <div>
                  <p className="text-[11px] font-medium text-gray-400 mb-1.5">Papel</p>
                  <div className="grid grid-cols-2 gap-1.5">
                    {(['', '1', '4', '2', '8'] as const).map(v => (
                      <button key={v} onClick={() => setF('papel', v)}
                        className={cn('text-xs py-1.5 rounded-md border transition-colors',
                          filtros.papel === v
                            ? 'bg-[#3B82F6] border-[#3B82F6] text-white'
                            : 'border-gray-200 text-gray-500 hover:border-gray-300')}>
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                {search || filtrosAtivos
                  ...
                  : 'Clique em "Nova Pessoa" para adicionar o primeiro cadastro.'}
              </p>
            </div>
            {!search && !filtrosAtivos && (
              ...
              <table className="w-full text-sm min-w-[700px]">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/60">
                    ...
                    <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide py-2.5 pr-6">Papel</th>
                      ...
                      <td className="py-3 pr-6">
                        <div className="flex flex-wrap gap-1">
                          {p.papeis.length > 0
                            ? p.papeis.map(papel => (
                                <span key={papel}
                                  className={cn('text-xs px-2 py-0.5 rounded-full font-medium', PAPEL_COLORS[papel] ?? 'bg-gray-100 text-gray-500')}>
                                  {papel}
                                </span>
                              ))
