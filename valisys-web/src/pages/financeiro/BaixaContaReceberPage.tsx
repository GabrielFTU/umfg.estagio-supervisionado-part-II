...
<div class="meta">
  <div><span>Data de recebimento:&nbsp;</span>${fmtDate(recibo.dataLancamento)}</div>
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100">
          <div>
            <h2 className="text-base font-semibold text-gray-800">Comprovante de Recebimento</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-right pb-3 px-3 text-xs font-semibold text-gray-500">Valor recebido</th>
...
export function BaixaContaReceberPage() {
  ...
  // When switching to Boleto, reset all valorAPagar to valorAberto
  useEffect(() => {
    if (isBoleto) {
      setParcelas(prev => prev.map(p => ({ ...p, valorAPagar: numToBRL(p.valorAberto) })));
    }
  }, [isBoleto]);
  ...
  useEffect(() => {
    if (!formaOpen) return;
    const h = (e: MouseEvent) => {
      if (formaRef.current && !formaRef.current.contains(e.target as Node))
        setFormaOpen(false);
    };
    document.addEventListener('mousedown', h);
    return () => document.removeEventListener('mousedown', h);
  ...
  const validate = () => {
    ...
    for (const p of parcelas) {
      ...
      if (principal > p.valorAberto) {
        ...
        errs[`valor-${p.parcelaId}`] = 'Recebimento à vista deve quitar o valor integral da parcela.';
      }
    }
    setErros(errs);
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      showToast(e.message ?? 'Erro ao registrar recebimento');
    } finally {
      setSaving(false);
    }
  };
  ...
  const selectedCart  = carteiras.find(c => c.id === carteiraId);
  const selectedForma = FORMAS_PAGAMENTO.find(f => f.value === formaPagamento);
  ...
  return (
    <>
      {recibo && (
        <ReciboModal
          recibo={recibo}
          onClose={() => { setRecibo(null); navigate('/financeiro/contas-receber'); }}
        />
      )}
      ...
      <div className="flex flex-col h-full bg-white">
        <div className="flex-1 overflow-auto px-10 py-8">
          ...
          <nav className="flex items-center gap-1.5 text-sm mb-10 text-gray-400">
            ...
            <ChevronRight size={12} className="shrink-0" />
            <span className="text-emerald-600 font-medium">Recebimento</span>
          </nav>
          ...
          <div className="flex items-start gap-10 mb-10 flex-wrap">
            ...
            <div className="relative w-44" ref={formaRef}>
              ...
              {isBoleto && (
                <p className="text-[11px] text-amber-600 mt-0.5">Valor bloqueado pelo boleto</p>
              )}
              {formaOpen && (
                <div className="absolute z-30 mt-1 left-0 w-full bg-white border border-gray-200 rounded-xl shadow-lg py-1 text-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-right py-3 px-3 text-xs font-semibold text-gray-600">Valor a receber</th>
                  ...
                  <th className="text-right py-3 px-3 text-xs font-semibold text-gray-600">Total recebido</th>
                </tr>
              </thead>
              <tbody>
                {parcelas.map(p => (
